...
public class VelocityServerListener {
    ...
    public void onPlayerChat(PlayerChatEvent event) {
        ...
        if (!Helper.playerCanChat(plugin, player.getUniqueId(), player.getUsername()))
            return;

        String finalPlayerMessage = playerMessage;
        event.getPlayer().getCurrentServer().ifPresent((connection) -> {
            VelocityChatMessageData messageData = new VelocityChatMessageData(plugin, MessageType.CHAT,
                    connection.getServer(), player, finalPlayerMessage);
    ...
    public void onDisconnect(DisconnectEvent event) {
        if (playerIsInDisabledServer(event.getPlayer(), plugin))
            return;
        if (plugin.isVanishAPIEnabled() && VelocityVanishAPI.isInvisible(event.getPlayer()))
    ...
    public void kickedFromServerEvent(KickedFromServerEvent event) {
        ...
        if (result.toString().contains("velocity.error.cant-connect"))
            return;
    ...
    protected void leave(Player player, String serverName) {
        chatHandler.runProxyLeaveMessage(
                ...
                player.getUniqueId(),
                serverName,
                (message, permission) -> sendToAllServersLeaveFiltered(message, permission, player.getUniqueId(),
                        serverName));
    }
    ...
    protected void join(Player player, String serverName) {
        chatHandler.runProxyJoinMessage(
                ...
                player.getUniqueId(),
                serverName,
                (message, permission) -> sendToAllServersJoinFiltered(message, permission, player.getUniqueId(),
                        serverName));
    }
    ...
    private void startServerStatusDetection() {
        ...
        plugin.getProxyServer().getScheduler()
                .buildTask(plugin, () -> plugin.getProxyServer().getAllServers().forEach((registeredServer) -> {
                    String serverName = registeredServer.getServerInfo().getName();

                    registeredServer.ping().thenAccept((ping) -> { // Server is online.
                        this.serverStatusManager.runStatusLogic(serverName, true, plugin.getDiscordBot(),
                                plugin.getLogger()::info);
                    }).exceptionally((exception) -> { // Server is offline.
                        this.serverStatusManager.runStatusLogic(serverName, false, plugin.getDiscordBot(),
                                plugin.getLogger()::info);
                        return null;
                    });
    ...
    public void onLoginEvent(LoginEvent event) {
        if (!plugin.getConfig().get(ConfigKey.USE_SIMPLE_PROXY_CHAT_BANNING_SYSTEM).asBoolean())
            return;

        UUID playerUUID = event.getPlayer().getUniqueId();
        if (!plugin.getBanHelper().isBanned(playerUUID))
            return;

        event.setResult(ResultedEvent.ComponentResult
                .denied(Helper.stringToComponent("&cYou are banned from the proxy.")));
    }
    ...
    public void onServerConnected(ServerConnectedEvent event) {
        ...
        // Event-driven: Instantly confirm the server is online when a player connects
        String connectedServerName = event.getServer().getServerInfo().getName();
        this.serverStatusManager.confirmServerOnline(connectedServerName, plugin.getDiscordBot(),
                plugin.getLogger()::info);

        // First Join
        if (event.getPreviousServer().isEmpty()) {
            if (playerIsInDisabledServer(event.getServer().getServerInfo(), plugin))
                return;
     ...
     * @param subjectUUID       The UUID of the subject player (for exclude-self
     *                          filtering)
     * @param subjectServerName The server name of the subject (for exclude-server
     *                          filtering)
    ...
    private void sendToAllServersFiltered(String message, Permission permission, UUID subjectUUID,
        ...
        plugin.getProxyServer().getAllPlayers().stream()
                ...
                // Ensure the subject is not included in the stream when excluding the server to
                // avoid duplicates
        ...
        // If excluding the server but not the subject, explicitly send to the subject
        // to keep behavior consistent
        if (excludeServer && !excludeSelf) {
            plugin.getProxyServer().getPlayer(subjectUUID).ifPresent((subjectPlayer) -> {
                if (plugin.getConfig().get(ConfigKey.USE_PERMISSIONS).asBoolean()
                        && !subjectPlayer.hasPermission(permission.getPermissionNode()))
                    return;
                if (playerIsInDisabledServer(subjectPlayer, plugin))
                    return;
    ...
    private void sendToAllServersJoinFiltered(String message, Permission permission, UUID subjectUUID,
            String subjectServerName) {
    ...
    private void sendToAllServersLeaveFiltered(String message, Permission permission, UUID subjectUUID,
            String subjectServerName) {
        sendToAllServersFiltered(message, permission, subjectUUID, subjectServerName,
